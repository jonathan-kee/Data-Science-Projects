CREATE TABLE raw.recipe_output_aggregates AS
with
    joining_table as (
        ...
        -- Because you only join on "materialoutput", you cannot count the profit of output
        inner join
            raw.stg_prices
    ...
    group_by_total as (
        ...
        CAST(max("minutes") / max("materialoutputquantity") AS INT) % 60, ' mins'
            ...
            sum("AI1-BidPrice") as "total output_materials_AI1_BidPrice"
            ,
            sum("AI1-BidPrice") / max("materialinputquantity") as "output_materials_AI1_BidPrice_per_unit"
        from joining_table
        group by
            joining_table."original_query",
            joining_table."prefix"
