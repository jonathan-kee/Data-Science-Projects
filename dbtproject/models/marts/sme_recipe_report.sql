...
select
    ...
    -- 1.1248 is based on expert calculation --
    ((1440 / recipe_table_input_aggregate."minutes per order") * (1.1248)) as "expert_order_per_day",
    ((1440 / recipe_table_input_aggregate."minutes per order") * (1.1248)) 
    * 25.00 as "production_fee_per_day",
    ((1440 / recipe_table_input_aggregate."minutes per order") * (1.1248)) 
    * ("total output_materials_AI1_BidPrice") as "revenue_per_day",
    -- 25 is production fee --
    ((1440 / recipe_table_input_aggregate."minutes per order") * (1.1248))  * ("total output_materials_AI1_BidPrice" - "total input_materials_AI1_AskPrice")
    - ((1440 / recipe_table_input_aggregate."minutes per order") * (1.1248))  * 25.00 as "profit_per_day",
